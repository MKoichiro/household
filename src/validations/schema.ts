...
export const transactionSchema = z.object({
  ...
  date: z
    .string()
    .nonempty({ message: "日付は必須です" }),
  amount: z
    .string()
    .nonempty({ message: "金額は必須です" })
    .refine(val => val !== "0", {message: "金額は1円以上で入力してください"})
    .refine(val => !isNaN(parseIntFromCommaSeparated(val)), {message: "無効な値です"}),
  content: z
    .string()
    .nonempty({ message: "内容は必須です" })
    .max(50, "内容は50文字以内にしてください"),
  category: z
    .union([
